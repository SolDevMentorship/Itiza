...
const ResetPasswordModal: React.FC<Props> = ({ isOpen, onClose, prefillEmail = "" }) => {
  ...
  if (!mounted || !portalNodeRef.current) return null;

  return createPortal(
    <Dialog open={isOpen} onClose={handleClose} className="fixed inset-0 z-[99999] overflow-y-auto" aria-label="Reset password dialog">
      {/* Backdrop */}
          ...
          {message && (
            <div
              className={`mb-4 p-3 rounded-lg text-sm ${
                message.type === "error" ? "bg-red-100 border border-red-400 text-red-700" : "bg-green-50 border border-green-200 text-green-800"
              }`}
            >
              {message.text}
            </div>
          ...
          {step === "email" ? (
            ...
            >
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-pink-200 rounded-lg text-center text-xl font-mono tracking-wider mb-3"
                disabled={loading}
                inputMode="numeric"
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-pink-200 rounded-lg mb-3"
                disabled={loading}
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-pink-200 rounded-lg mb-4"
                disabled={loading}
                autoComplete="new-password"
              ...
              <button
                ...
                className="w-full mt-3 py-2 text-gray-600 hover:text-gray-800 text-sm underline"
                disabled={loading}
              >
                ← Back
              </button>
