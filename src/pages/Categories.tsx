...
import GiftModal, { GiftItem } from "@/components/GiftModal";
...
const Categories: React.FC = () => {
  ...
  const initialSearchResults: GiftResult[] | undefined = locationState.searchResults;

  const [allGifts, setAllGifts] = useState<GiftResult[]>([]);
  const [searchResults, setSearchResults] = useState<GiftResult[] | undefined>(initialSearchResults);
  const [query, setQuery] = useState<string>(locationState.query ?? "");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          // normalize shape
          const mapped: GiftResult[] = (Array.isArray(data) ? data : []).map((r: any) => ({
  ...
  useEffect(() => {
    ...
    if (state.searchResults && Array.isArray(state.searchResults)) {
      setSearchResults(state.searchResults as GiftResult[]);
  ...
  const openGiftModal = (gift: GiftResult) => {
    // Coerce giftID to string|number (GiftModal expects non-nullable giftID)
    const giftIDValue: string | number =
      gift.giftID != null ? gift.giftID : gift.id != null ? String(gift.id) : "";

    const modalPayload: GiftItem = {
      giftID: giftIDValue,
      merchantID: gift.merchantID ?? null,
      name: gift.name ?? "Untitled Gift",
      price: typeof gift.price === "number" ? gift.price : gift.price ? Number(gift.price as any) : null,
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8 md:py-16">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {searchResults && searchResults.length > 0 && (
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={clearSearch}
                className="text-sm py-2 px-3 rounded-md border border-gray-200 hover:bg-gray-50 text-gray-700"
        ...
        {loading ? (
          <div className="py-20 text-center text-gray-500">Loading gifts…</div>
        ) : error ? (
          <div className="py-20 text-center text-red-500">Error: {error}</div>
        ) : renderList.length === 0 ? (
          <div className="py-20 text-center text-gray-500">No gifts available.</div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="aspect-w-1 aspect-h-1 bg-gray-50">
                    <img
                      src={gift.img ?? "/images/placeholder.png"}
                      alt={gift.name ?? "Gift"}
                  ...
                  <div className="p-4">
                    ...
                    {typeof gift.price !== "undefined" && gift.price !== null && (
                      <div className="mt-3 font-semibold text-pink-600">${Number(gift.price).toFixed(2)}</div>
                    )}
                  </div>
                </div>
              );
