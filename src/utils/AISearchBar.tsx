...
import SearchResults, { GiftSearchResult } from "./SearchResults";
 ...
 */
const parseNumberIfPossible = (v: any): number | string | null => {
...
const fetchGiftsFromServer = async (query: string): Promise<GiftSearchResult[]> => {
  ...
  return data.map((r: any) => {
    ...
    const giftID: string | number = parsedGiftID == null ? "" : parsedGiftID;

    const mapped: GiftSearchResult = {
      ...
      stockQuantity: parseNumberIfPossible(
        r.stockQuantity ?? r.stock_quantity ?? r.raw?.stockQuantity ?? null
      ),
      ...
      ...(r ? { raw: r } : {}),
    } as unknown as GiftSearchResult;
...
const AISearchBar: React.FC = () => {
  ...
  const [results, setResults] = useState<GiftSearchResult[]>([]);
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.trim().length > 0) {
      // debounce 300ms
      searchTimeout.current = window.setTimeout(() => runSearch(val), 300) as unknown as number;
    } else {
      setResults([]);
      setShowResults(false);
      setIsLoading(false);
  ...
  const handleSelectResult = (item: GiftSearchResult) => {
    navigate("/categories", {
      state: {
        searchResults: [item],
        query: query.trim(),
    ...
    <div ref={searchContainerRef} className="relative w-full max-w-xl mx-auto">
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-2 pl-10 pr-12 text-gray-700 bg-white border rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
          onKeyDown={(e) => {
        ...
        {isLoading ? (
          <Loader2 className="absolute left-3 top-3 text-blue-500 animate-spin" size={18} />
        ) : (
        ...
        <button
          ...
          aria-label="Open results in categories"
        ...
        <SearchResults
          ...
          onSelect={(selected) => {
            ...
            const matched = results.find((r) => {
              if (r.giftID != null && selId != null) return String(r.giftID) === String(selId);
            ...
            const rawAny = (selected as any).raw ?? null;
                ...
                searchResults: [
                  {
                    ...
                    name: selected.name ?? (selected as any).title ?? "",
