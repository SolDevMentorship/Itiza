...
interface GiftRow {
  ...
  resolvedBuffer?: Buffer | null;
}

const ALLOWED_ORDER_KEYS: (keyof OrderPayload)[] = [
  "trackingID",
  "customerID",
  "merchantID",
  "giftID",
...
function maskPII(obj: any) {
  ...
  if (copy.phone || copy.phonenumber || copy.recipientPhone) {
    if (copy.phone) copy.phone = copy.phone.replace(/\d(?=\d{3})/g, "*");
    if (copy.phonenumber) copy.phonenumber = copy.phonenumber.replace(/\d(?=\d{3})/g, "*");
    if (copy.recipientPhone) copy.recipientPhone = copy.recipientPhone.replace(/\d(?=\d{3})/g, "*");
  }
  return copy;
}
 ...
 */
async function resolveGiftImage(
  supabase: ReturnType<typeof getSupabase>,
  g: GiftRow
): Promise<{ publicUrl?: string | null; buffer?: Buffer | null }> {
  const candidate = (g.img ?? g.imageUrl) as string | null | undefined;
  if (!candidate) return {};
  ...
  for (const bucket of buckets) {
    try {
      ...
      if (!dlErr && downloaded) {
        try {
          ...
          try {
            // @ts-ignore
            if (Buffer.isBuffer(downloaded)) return { buffer: downloaded as Buffer };
          } catch {}
        }
      }
    } catch (e) {
    ...
    try {
      const maybe = (supabase.storage.from(bucket).getPublicUrl(candidate) as any) || {};
      const publicURL =
        (maybe.data && (maybe.data.publicUrl ?? maybe.data.publicURL)) ?? maybe.publicUrl ?? maybe.publicURL ?? null;
      if (publicURL) return { publicUrl: publicURL };
    } catch (e) {
      // ignore
    }
  }
...
async function sendOrderEmail(options: {
  ...
  if (gift) {
    ...
    if (gift.resolvedBuffer && Buffer.isBuffer(gift.resolvedBuffer)) {
      ...
      inlineImgHtml = `<div style="margin-top:12px;text-align:center"><img src="cid:${cid}" alt="${escapeHtml(
        giftName
      )}" style="max-width:240px;height:auto;border-radius:8px;border:1px solid #f0f0f0;"/></div>`;
    } else if (gift.resolvedImg) {
      inlineImgHtml = `<div style="margin-top:12px;text-align:center"><img src="${escapeHtml(
        gift.resolvedImg
      )}" alt="${escapeHtml(giftName)}" style="max-width:240px;height:auto;border-radius:8px;border:1px solid #f0f0f0;"/></div>`;
    }
  }
  ...
  const html = `
    <div style="font-family: Inter, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial; color: #111; line-height:1.5; max-width:700px">
      <div style="background:#fff;border-radius:10px;padding:20px;border:1px solid #f3f4f6">
        ...
        <p style="margin-top:14px;color:#334155">If you have any questions, please <a href="https://itiza.co/contact">contact us</a> or email <a href="mailto:itiza.co@gmail.com">itiza.co@gmail.com</a>. For faster service, include your reference number <strong>${escapeHtml(
          trackingID
        )}</strong>.</p>
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const rawRows: any[] = Array.isArray(body) ? body : [body];

    const validationErrors: string[] = [];
    const cleanRows: OrderPayload[] = rawRows.map((r, idx) => {
      ...
      for (const key of ALLOWED_ORDER_KEYS) {
        if (r[key as string] !== undefined) picked[key] = r[key as string];
      }

      if (!picked.trackingID) picked.trackingID = randomUUID();
      return picked as OrderPayload;
    });
    ...
    const merchantIdsSet = new Set<string>();

    for (const ord of insertedOrders) {
      const cid = ord.customerID ?? "";
      if (!cid) continue;
      customerIdsSet.add(cid);
      if (ord.giftID != null) giftIdsSet.add(Number(ord.giftID));
      if (ord.merchantID) {
    ...
    const giftsMap = new Map<number, GiftRow>();
    if (giftIds.length > 0) {
    ...
    if (merchantIDs.length > 0) {
      ...
      if (merchantsErr) {
        ...
        console.log("[DEBUG orders] merchantsData returned:", merchantsData.map((m: any) => ({ merchantID: m.merchantID, email: m.email, businessName: m.businessName })));
    ...
    const toneQuery = (req.query.tone as string) || "friendly";
    const tone: "friendly" | "formal" = toneQuery === "formal" ? "formal" : "friendly";

    const sendPromises = insertedOrders.map(async (ord: any) => {
      const results: any = { order: ord, customerEmail: null, merchantEmail: null };
      try {
        ...
        if (merchantID) {
          ...
          if (!merchantRow || !merchantRow.email) {
            ...
            try {
              ...
              console.error(`[DEBUG orders] Error sending merchant email for order ${ord.trackingID} to merchantID=${merchantID}`, (me && typeof me === "object" && "message" in me ? (me as any).message : me));
