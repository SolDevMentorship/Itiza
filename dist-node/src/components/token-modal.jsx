...
export default function GiftTokenModal({ isOpen, onClose }) {
    ...
    const filteredTokens = tokenList.filter(token => token.symbol.toLowerCase().includes(tokenQuery.toLowerCase()));
    const handleSubmit = async (e) => {
        ...
        try {
            const transferSuccessful = await invokeGiftToken(connection, wallet, amount, giftAddress, new PublicKey(SOL_MINT), new PublicKey(selectedTokenMint));
            ...
            <DialogContent className="sm:max-w-md bg-pink-50">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 py-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input placeholder="Enter wallet address" value={giftAddress} onChange={(e) => setGiftAddress(e.target.value)} disabled={isProcessing}/>
                    </div>

                    <div className="space-y-2">
                        <Label>Amount (SOL)</Label>
                        <Input type="number" step="any" placeholder="0.00" value={solAmount} onChange={(e) => setSolAmount(e.target.value)} disabled={isProcessing}/>
                    </div>

                    <div className="space-y-2">
                        <Label>Search Token Symbol</Label>
                        <Input placeholder="Start typing a token symbol..." value={tokenQuery} onChange={(e) => setTokenQuery(e.target.value)} disabled={isProcessing}/>
                        {filteredTokens.length > 0 && (<select value={selectedTokenMint || ""} onChange={(e) => setSelectedTokenMint(e.target.value)} className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm">
                                {filteredTokens.map((token) => (<option key={token.mint} value={token.mint}>
                                        {token.symbol}
                                    </option>))}
                            </select>)}
                    </div>

                    {solAmount && (<div className="p-3 bg-white/50 rounded-xl text-[#832c2c]/80 text-sm">
                    ...
                    {message && (<div className={`text-center ${message.includes("successfully") ? "text-green-600" : "text-red-500"}`}>
                            {message}
                        </div>)}
                    ...
                    <DialogFooter>
                        ...
                        <Button type="submit" disabled={isProcessing || !wallet.connected || tokenList.length === 0} className="bg-pink-600 hover:bg-pink-700 text-white">
                            {isProcessing ? (<>
