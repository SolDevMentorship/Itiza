...
export default function GiftModal({ item, isOpen, onClose }) {
    const { publicKey, connected, signTransaction } = useWallet();
    // Persisted customer info (from /auth/me or localStorage)
    ...
    const totalPayable = Number((computedAmount + networkFee).toFixed(2));
    const totalPayableSol = convertUsdToSol(totalPayable);
    const getCustomerIDFromStorage = () => customerID ||
        ((() => {
            try {
    ...
    const getCustomerEmailFromStorage = () => customerEmail || getCustomerIDFromStorage();
    //
    // Recipient validation
    //
    ...
    const saveOrderToBackend = async (payload) => {
        try {
            console.log("[orders] saving payload:", payload);
            const VITE_API_URL = import.meta.env.VITE_API_URL || "https://itiza-backend.vercel.app";
