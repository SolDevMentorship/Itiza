...
function convertUsdToSol(usdAmount) {
...
export default function GiftModalOld({ item, isOpen, onClose }) {
    const { publicKey, connected, signTransaction } = useWallet();
    // Persisted customer info (from /auth/me or localStorage)
    const [customerID, setCustomerID] = useState(null);
    const [customerEmail, setCustomerEmail] = useState(null);
    const [_customerPhone, setCustomerPhone] = useState(null);
        ...
        (async () => {
            try {
                ...
                if (!mounted)
                    return;
                if (user) {
                    const id = (user.customerID || user.email || user.identifier || null);
                    if (id) {
                        setCustomerID(id);
                        try {
    ...
    const getCustomerIDFromStorage = () => customerID || localStorage.getItem("userIdentifier") || localStorage.getItem("userCustomerID") || localStorage.getItem("userEmail") || null;
    const getCustomerEmailFromStorage = () => customerEmail || getCustomerIDFromStorage() || localStorage.getItem("userEmail") || null;
    //
    ...
    const recipientValidationError = () => {
        ...
        if (!trim(recipientPhone))
            return "Recipient phone is required";
        if (!/^\d{7,15}$/.test(recipientPhone))
            return "Recipient phone must be digits only (7-15 digits)";
        if (!trim(recipientStreet))
            return "Recipient street is required";
        if (!trim(recipientCity))
            return "Recipient city is required";
        if (!trim(recipientState))
            return "Recipient state is required";
        if (!trim(recipientCountry))
            return "Recipient country is required";
        if (!selectedRelation)
            return "Please select your relationship to the recipient";
        if (!quantity || quantity <= 0)
            return "Quantity must be at least 1";
        if (unitPrice <= 0)
            return "Invalid gift price";
        return null;
    };
    ...
    const saveOrderToBackend = async (payload) => {
        try {
            ...
            if (!resp.ok) {
                ...
                throw new Error(`Database insert failed (status ${resp.status}) - ${JSON.stringify(bodyJson ?? bodyText)}`);
            }
    ...
    const resolveGiftID = () => {
        ...
        if (typeof raw === "number") {
            if (!Number.isFinite(raw))
                throw new Error("giftID is not a finite number");
    ...
    const validateOrderInputs = () => recipientValidationError();
    ...
    const handleCryptoPayment = async () => {
        ...
        try {
            ...
            const tx = new Transaction({ recentBlockhash: blockhash, feePayer: publicKey }).add(SystemProgram.transfer({
    ...
    const handleCardPayment = async () => {
        ...
        try {
            ...
            const shouldContinue = confirm(`You will be charged ₦${ngnAmount.toLocaleString()} NGN (approximately $${totalPayable.toFixed(2)} USD). Continue?`);
            if (!shouldContinue) {
                setLoading(false);
                return;
            }
            ...
            if (!verified) {
                const msg = lastVerifyResult?.body?.error ||
    ...
    const onRecipientPhoneChange = (value) => {
    ...
    return (<>
          ...
          <div className="grid md:grid-cols-2 gap-6 p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {relationships.map((rel) => (<button key={rel} onClick={() => setSelectedRelation(rel)} className={`px-3 py-1.5 rounded-full text-sm transition-colors ${selectedRelation === rel ? "bg-[#e47a7a] text-white" : "bg-[#fce8e6] text-[#832c2c] hover:bg-[#f6c1c1]"}`}>
                    {rel}
                  </button>))}
              </div>

              <textarea value={giftMessage} onChange={(e) => setGiftMessage(e.target.value)} placeholder="Write a message to the recipient..." className="w-full h-20 p-3 rounded-xl bg-white/50 border border-[#f6c1c1] focus:border-[#e47a7a] resize-none"/>
              ...
              <div className="space-y-2">
                ...
                <input className="w-full p-3 rounded-xl border border-[#f6c1c1]" value={recipientPhone} onChange={(e) => onRecipientPhoneChange(e.target.value)} placeholder="Digits only, e.g. 2348012345678" inputMode="tel"/>
              ...
              <div className="flex items-center gap-3">
                <div className="w-24">
                  ...
                  <input type="number" min={1} value={quantity} onChange={(e) => setQuantity(Math.max(1, Number(e.target.value)))} className="w-full p-3 rounded-xl border border-[#f6c1c1]"/>
                </div>
              ...
              <div className="flex gap-3 mt-2">
                ...
                <Button onClick={onAttemptOpenPaymentChoices} className="flex-1 bg-[#e47a7a] text-white" disabled={!isRecipientFormValid || loading}>
      ...
      {/* Payment Choice */}
      {showPaymentChoice && (<div className="fixed inset-0 z-[60] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 transform transition-all">
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => setShowCardModal(true)} className="w-full flex items-center justify-between px-4 py-3 rounded-lg border border-[#f6c1c1] bg-white text-[#832c2c] font-medium hover:bg-[#fff2f2]">
                <div>
      ...
      {showCardModal && (<div className="fixed inset-0 z-[70] flex items-center justify-center">
      ...
      {showLoginModal && (<LoginModal isOpen={showLoginModal} onClose={() => setShowLoginModal(false)} onSwitchToSignup={() => setShowLoginModal(false)} onSwitchToResetPasswordModal={() => setShowLoginModal(false)} setIsLoggedIn={(v) => setIsLoggedIn(v)} setUserFullName={() => { }} disableClose={true} hideSecondaryActions={true}/>)}
    </>);
}
