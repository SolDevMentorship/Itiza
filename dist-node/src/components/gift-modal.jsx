...
export default function GiftModal({ isOpen, onClose }) {
    ...
    const [selectedToken, setSelectedToken] = useState(TOKEN_LIST[0].address);
            ...
            <DialogContent className="sm:max-w-md bg-pink-50">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 py-4">
                    <div className="space-y-2">
                        ...
                        <Input placeholder="1234567890" value={phoneNumber} onChange={handlePhoneChange} disabled={isProcessing}/>
                    </div>

                    <div className="space-y-2">
                        <Label>Amount</Label>
                        <Input type="number" placeholder="10.00" value={amount} onChange={(e) => setAmount(e.target.value)} disabled={isProcessing}/>
                    </div>

                    <div className="space-y-2">
                        <Label>Payment Token</Label>
                        <select value={selectedToken} onChange={(e) => setSelectedToken(e.target.value)} className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
                            {TOKEN_LIST.map((token) => (<option key={token.address} value={token.address}>
                                    {token.label}
                                </option>))}
                        </select>
                    </div>
                    ...
                    <DialogFooter>
                        ...
                        <Button type="submit" disabled={isProcessing || !wallet.connected} className="bg-pink-600 hover:bg-pink-700 text-white">
                            {isProcessing ? (<>
