...
export const invokeGiftToken = async (connection, wallet, giftAmount, // e.g., 1.00 USDC (human-readable)
recipientAddress, // Base58 string
mintToPayWith, tokenMint) => {
    try {
        ...
        const recipientATA = await getAssociatedTokenAddress(mintPubkey, recipientPubkey, false // allowOwnerOffCurve
        );
        // Check if ATA exists
        ...
        if (!accountInfo) {
            const ataIx = createAssociatedTokenAccountInstruction(wallet.publicKey, // payer
        ...
        const senderATA = await getAssociatedTokenAddress(mintPubkey, senderPubkey, false // allowOwnerOffCurve
        );
        ...
        if (!senderAccountInfo) {
            const senderAtaIx = createAssociatedTokenAccountInstruction(wallet.publicKey, // payer
