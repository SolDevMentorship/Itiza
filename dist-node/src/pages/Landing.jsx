...
export default function LandingPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                if (user.customerID)
                    localStorage.setItem("userCustomerID", user.customerID);
                if (user.email)
    ...
    return (<div className="min-h-screen bg-gradient-to-b from-pink-50 to-white">
      <Hearts />
      <Clouds />
      <LandingHeader isLoggedIn={isLoggedIn} setIsLoggedIn={setIsLoggedIn} setShowLoginModal={setShowLoginModal} setShowSignUpModal={setShowSignUpModal}/>

      <section className="relative pt-20 pb-32 overflow-hidden">
        <div className="container mx-auto px-4">
          <div className="flex flex-col lg:flex-row items-center">
            <div className="lg:w-1/2 mb-12 lg:mb-0">
              {isLoggedIn && (<motion.div className="text-2xl md:text-3xl font-semibold text-left text-rose-500 mb-10 pl-2 md:pl-0 -mt-6 md:-mt-4" style={{ fontFamily: "cursive" }}>
                  Hi, {firstName}
                </motion.div>)}
              ...
              <motion.div>
                <Button className="bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 text-white px-8 py-6 rounded-full text-lg font-medium" onClick={() => handleProtectedNavigation("/dashboard")}>
            ...
            <div className="lg:w-1/2">
              <motion.div className="relative">
                <div className="bg-white p-4 rounded-2xl shadow-xl transform rotate-3 relative z-10 overflow-hidden w-[500px] h-[700px]">
                  <img src="/images/gold-ring.png" alt="Gift unwrapping experience" className="w-full h-full object-cover rounded-lg"/>
                </div>
      ...
      <section className="py-20 bg-pink-50">
        <div className="container mx-auto px-4">
          <div className="grid md:grid-cols-3 gap-8">
            {benefits.map((b) => (<div key={b.id} className="bg-white rounded-xl p-6 shadow">
      ...
      <LoginModal isOpen={showLoginModal} onClose={() => setShowLoginModal(false)} onSwitchToSignup={() => {
        ...
        }} setUserFullName={(name) => setUserFullName(name)}/>
      ...
      <ResetPasswordModal isOpen={showResetPasswordModal} onClose={() => setShowResetPasswordModal(false)} prefillEmail={localStorage.getItem("prefillEmail") ?? undefined}/>
    </div>);
}
