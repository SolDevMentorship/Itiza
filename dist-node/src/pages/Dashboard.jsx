...
export default function Dashboard() {
    ...
    useEffect(() => {
        const fetchGifts = async () => {
            try {
                ...
                const urlBase = `${import.meta.env.VITE_API_URL}/Itiza_Delivery/gifts`;
    ...
    const truncate = (text, wordLimit = 10) => {
      ...
      <main className="container mx-auto px-4 py-12">
          ...
          <div className="flex flex-col lg:flex-row items-start justify-between gap-8">
            ...
            <Card className="bg-gradient-to-br from-blue-600 to-purple-600 text-white w-full lg:w-[320px] shadow-xl">
              <CardContent className="p-4 flex flex-col justify-between h-full">
                ...
                <Button variant="ghost" className="mt-auto text-blue-100 hover:text-white hover:bg-white/10 text-sm px-3 py-2" onClick={() => window.open("https://ryder.id/products/ryder-one", "_blank")}>
                  Learn More →
                </Button>
              </CardContent>
            </Card>
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            ...
            return (<Card key={String(gift.giftID)} onClick={() => handleItemClick(gift)} className="group bg-white/40 hover:bg-white/60 transition-all cursor-pointer border-0 shadow-md hover:shadow-xl rounded-lg overflow-hidden hover:-translate-y-1 relative">
                  <CardContent className="p-3">
                    <div className="relative aspect-square mb-2 overflow-hidden rounded-md">
                      <img src={gift.img} alt={gift.name} className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-300"/>
                      {outOfStock && (<div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
        ...
        {/* Modals */}
        {selectedGift && (<GiftModalOld item={{
